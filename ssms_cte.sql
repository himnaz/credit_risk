-- Convert the entire script to use CTEs instead of temporary tables
WITH 
    ...
    FROM (
        ...
        FROM (
            SELECT DISTINCT 
                ...
                UC.UC_BusinessArea,
                CASE WHEN UC.UC_BusinessArea = 3 THEN CONVERT(DATE, UC.UC_DateAchieved, 103) ELSE '' END as [Date Achieved]
...
NI_ID AS (
    SELECT 
        ...
        CASE WHEN STAF_NINumber IS NULL 
             ...
             ELSE STAF_NINumber 
        END AS ID,
        ISNULL(ah.AgentLeftDate, '01-Jan-3030') as AgentLeftDate
    FROM [dbo].[sa_Insight_Staff] S 
    JOIN dbo.sa_insight_UserDetails UD ON UD.USR_ID = S.STAF_UserID 
    JOIN cscv_AgentsHierarchy ah ON USR_ExternalID = ah.AgentCode AND (ah.ASDCode <> '4019902' OR ah.FirmCode <> '2016789')
    WHERE ah.CompetentStatusDescription IN('Left', 'Active')
        AND (rtrim(ah.ASDName) IN ('Pat Mckenna', 'Owl Sales Director') OR ah.ASDName IS NULL)
),
...
Panel AS (
    ...
    INNER JOIN (
        SELECT 
            ...
            Panel,
            EffFrom,
            ISNULL(EffTo, '31 dec 2999') as EffTo 
        FROM [dbo].[f_Insight_ListPanelHistoryOfAllSellers]()
             ...
             END = CASE WHEN LEN(ag.Code) = 4 THEN CASE WHEN LEFT(ag.Code,2) = '00' THEN '00' + ag.Code
                       ...
                       ELSE ag.CODE 
                  END 
             AND DATEADD(dd, 0, DATEDIFF(dd, 0, GETDATE())) BETWEEN INS.EffFrom AND INS.EffTo
),
...
-- CTE for Adviser Table
AdviserTable AS (
    SELECT DISTINCT 
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ah.AgentCode)), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Adviser UID],
        LTRIM(RTRIM(ah.AgentDesc)) as [Adviser Name],
        ah.CompetentStatusdescription as [Swift Status],
        ISNULL(CD.Code_Description, '') as [Insight Status],
        ...
        ISNULL(ag.ReasonForLeaving, '') as [Reason For Leaving],
        CASE WHEN ah.AgentTypeDescription = 'Franchise' THEN ah.AgentCode ELSE ah.FirmCode END as [Firm UID],
        LTRIM(RTRIM(ah.FirmDescn)) as [Firm Name],
        ag3.descn as SDC,
        ISNULL(uh.Supervisorname, '') as [Supervisor Name],
        ISNULL(ah.rsmname, '') as [RSM Name],
        ISNULL(uh.BQMName, '') as [QRM Name],
        ISNULL(uh.AQMName, '') as [AQM Name],
        ISNULL(ah.ASDCode, '') as [ASD UID],
        ISNULL(ah.ASDname, '') as [ASD Name],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID1, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 1],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName1, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 1],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID2, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 2],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName2, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 2],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID3, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 3],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName3, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 3],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID4, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 4],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName4, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 4],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID5, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 5],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName5, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 5],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID6, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 6],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName6, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 6],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID7, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 7],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName7, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 7],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID8, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 8],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName8, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 8],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID9, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 9],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName9, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 9],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMUID10, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM UID 10],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(uh.OwlASMName10, ''))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Owl ASM Name 10],
        ...
        CASE WHEN cl.Gender = 'U' THEN 
                CASE WHEN ISNULL(CD2.Code_Description, '') = 'Female' THEN 'F'
             ...
             ELSE ISNULL(cl.Gender, CD2.Code_Description) 
        END as Gender,
        (DATEDIFF(d, cl.BIRTHDATE, GETDATE()) / 365) + 1 as Age_Next_Birthday,
        ...
        ISNULL(ag.email, cl.EMAIL) as Email,
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(s_cl.CorrAddress1, ag.address1))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Address Line 1],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(s_cl.CorrAddress2, ag.address2))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Address Line 2],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(s_cl.CorrAddress3, ag.address3))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Address Line 3],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(s_cl.CorrTown, ag.address4))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Address Line 4],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(s_cl.CorrCounty, ag.address5))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as [Address Line 5],
        REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(s_cl.CorrPostcode, ag.postcode))), CHAR(9), ''), CHAR(10), ''), CHAR(13), '') as Postcode,
    ...
    LEFT JOIN mi_insight_UserHierarchyallusers uh ON 
